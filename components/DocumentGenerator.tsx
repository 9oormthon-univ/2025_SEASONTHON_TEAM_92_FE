            ...
            if (line.trim()) {
              ...
              if (/[가-힣]/.test(line)) {
                ...
                const processedLine = line
                  .replace(/임차인/g, 'Tenant')
                  .replace(/임대인/g, 'Landlord')
                  .replace(/주소/g, 'Address')
                  .replace(/연락처/g, 'Contact')
                  .replace(/이메일/g, 'Email')
                  .replace(/작성일/g, 'Date')
                  .replace(/계약서/g, 'Contract')
                  .replace(/요구서/g, 'Request')
                  .replace(/내용증명/g, 'Content Notice')
                  .replace(/월세/g, 'Monthly Rent')
                  .replace(/조정/g, 'Adjustment');
                
                pdf.text(processedLine, x, currentY);
              } else {
                pdf.text(line, x, currentY);
              }
              currentY += fontSize * 0.4; // 줄 간격
            }
          });
        } catch (error) {
          // 오류 발생 시 기본 텍스트로 대체
          pdf.text('Document Content', x, y);
        }
      };
      
      // 제목 추가
      addText(title, 20, 30, 20, true);
      
      // 기본 정보 추가
      const currentDate = new Date().toLocaleDateString('ko-KR');
      addText(`Date: ${currentDate}`, 20, 50);
      addText(`Tenant: ${tenantInfo.name || '[Name]'}`, 20, 65);
      addText(`Tenant Address: ${tenantInfo.address || '[Address]'}`, 20, 80);
      addText(`Tenant Contact: ${tenantInfo.phone || '[Phone]'}`, 20, 95);
      addText(`Landlord Email: ${landlordEmail || '[Email]'}`, 20, 110);
      addText(`Property Address: ${reportData?.contractSummary?.address || '[Property Address]'}`, 20, 125);
      
      addText('This document is automatically generated by Monthly Rent Expert.', 20, 150);
      addText('Please consult with legal professionals before actual use.', 20, 165);
...
요청 사항: 월세 [현재금액] → [요청금액] 조정
      ...
      // 내용 추가 (영어로 처리)
      addText(content, 20, 190);
      
      // 서명란 추가
      pdf.setFontSize(12);
      pdf.text('Tenant Signature: _________________', 20, 250);
      pdf.text('Landlord Signature: _________________', 20, 270);
      
      // 하단 정보
      pdf.setFontSize(10);
      pdf.text('* This document is an AI-generated template and requires expert review for legal validity.', 20, 300);
      pdf.text('Monthly Rent Expert | https://rental-lovat-theta.vercel.app', 20, 310);
                ...
                <div className="bg-white border border-gray-200 rounded-lg p-4">
                  ...
                  <div className="mt-3 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                    <p className="text-sm text-blue-700">
                      ...
                      <strong>참고:</strong> PDF는 한글 폰트 제한으로 영어로 생성되며, Word 문서는 한글로 정상 생성됩니다.
                    </p>
                  </div>
                </div>
